...
class DialogManager:
    def __init__(self, agent, user, db_full, db_inc, movie_kb, verbose=True):
        self.agent = agent  # Агент типа act (ex: AgentActRule)
        self.user = user
    ...
    def initialize_episode(self):
        ...
        Инициализирует пользователя, пока его цель не ограничивает результаты поиска до SUCCESS_MAX_RANK шт
        Инициализирует агента
        '''
        while True:
            self.user_action: Dict = self.user.initialize_episode()
            if self._check_user_goal() <= dialog_config.SUCCESS_MAX_RANK: 
                break
        self.agent.initialize_episode()
        if self.verbose: 
            self.user.print_goal()
    ...
    def next_turn(self):
        ...
        # словарь с полем с макс. энтропием и др
        ...
        if episode_over: 
            self.agent.terminate_episode(self.user_action)

        if episode_over and self.verbose:
            print("Agent Results:")
            if 'phis' in self.sys_actions: 
                print('\t'.join(['dont-care:']+['%.3f'%s for s in self.sys_actions['phis']]))
